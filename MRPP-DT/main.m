...
while(any(TaskStatus))
    for i=1:robotNum
        ...
        if  NextRobotStates(i,1)==GoalRobotStates(i,1) &&  NextRobotStates(i,2)==GoalRobotStates(i,2)
            TaskStatus(i,1) = 0;
        end        
    end
...
for i = 1:intepolate+10
    plotAll(map,simTime,GoalRobotStates,GoalRobotStates,ArrivalTime,ColorMat,objectiveSelect);
    pause(0.1);
    frame = getframe;
    writeVideo(video,frame);
    cla;
