...
% to avoid overlap
for i=1:robotNum    
    plot(goalXY(i,1),goalXY(i,2),'o','MarkerEdgeColor',ColorMat(i,:),'MarkerFaceColor',[1 1 1],'MarkerSize',robotMarkerSize,'LineWidth',2);    
    ...
    txt=sprintf('%d',i);
    text(goalXY(i,1),goalXY(i,2),txt,'FontWeight','Bold','FontSize',fontSize,'HorizontalAlignment','center','Color',ColorMat(i,:));   
    hold on;
end
...
for i=1:robotNum
    ...
    if ArrivalTime(i,1)<=simT
        ...
        switch objectiveSelect
            ...
            case 6
                txt=sprintf('H_{%d}=%d',i,abs(ArrivalTime(i,1)-goalXY(i,3)));
            case 7
